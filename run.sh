...
if [[ ! -d "$PYTHON_VENV" ]]; then
    python3 -m venv "$PYTHON_VENV" || exit 1
fi
...
python "$WORK_DIR/src/check_version.py" || exit 1
...
if [[ "$(echo $?)" == 0 ]]; then
    ...
    python "$WORK_DIR/src/main.py" || exit 1
    MINECRAFT_VER=$(cat "$WORK_DIR/minecraft/server/version.txt")

    # Build the docker image
    echo -e "\nBuilding image..."
    docker build "$WORK_DIR/minecraft" -t "$IMAGE_NAME:$MINECRAFT_VER" || exit 1
